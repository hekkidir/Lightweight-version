...
### Changed
- **Strategy B (B-GATE-W3) retuned to its best mechanical stack** (from a 42-variant
  mechanics-only sweep in `backtest_lab`, evaluated on multi-window 2022-2026 consistency —
  see `backtest_lab/results/LEARNINGS.md`). Three B-specific changes, all leaving Strategy D
  untouched: (1) **liquidity floor $30M → $20M** (7-day-avg) — the floor is a U-shape and
  $20M is its optimum: $50M single-day kills the illiquid baggers (−48%), no floor lets bombs
  in and blows drawdown to −76%, $20M is the bottom (better return, same DD, better worst
  year); (2) **score floor 2.39 → 3.0** (more selective — its gain is concentrated in the
  2022 bear year); (3) **sizing C8a → C8-1.5** (steeper pyramid, 1/rank^1.5). On the
  2022-2026 window B goes ~+3136% → ~+5527% (Sharpe 1.36 → 1.51, worst year −22% → +8%, i.e.
  no losing year). The extra return concentrates in 2022 and 2025 (B's dominant year); 2023/24
  are ~flat. Knobs added to `config.ini [robots]` (`liq_min_sb_b`, `score_floor_b`) + typed in
  `RobotsConfig`; per-key overrides `LIQ_MIN_SB_BY_KEY` / `SCORE_FLOOR_BY_KEY` and a `C8-1.5`
  sizing variant in `robots.py`. D keeps $30M / 2.39 / C8a. `robots.json` is regenerated by
  the pipeline. Live current holdings shift to the new selection; H4 gate, dashboard score,
  Top-3, sector tilt, and graded vol-mult are unchanged (B is still B).

## [0.11.3] — 2026-07-05

### Fixed
- **Robot rebalance silently skipped any week whose Monday is an NYSE holiday.**
  `_simulate`'s weekly rebalance fired only on a literal `d.weekday() == 0`. When
  Monday is a holiday (MLK, Presidents, Memorial, Juneteenth, Independence Day,
  Labor Day, Christmas/New Year observed — 29 occurrences found across the
  2021-2026 price history, ~7/year), that entire week got **zero rotation**: no
  new entries, no rotation out of stale names, for every one of the 7 robots (the
  rebalance loop is shared, not SB-specific like F3). Rebalance now fires on the
  **first trading day of each ISO week** instead — a Monday holiday shifts that
  week's rotation to the next trading day (verified: 2024-01-15 MLK Monday absent
  from the data → 2024-01-16 Tuesday correctly becomes the week-start). Live
  picks are unchanged today (no holiday-adjacent day currently in play); the
  effect engages the next time a Monday holiday occurs.
  **Backtest-number caveat:** unlike F1/F3, this fix has no clean control group
  (it affects all 7 robots) and its full-window backtest deltas are **not a
  reliable performance signal** — every robot's trade list diverges within the
  first 2-15 trades (May-Aug 2022, the first holiday week in the sim), and 4+
  years of compounding weekly rotation cascades that single early divergence
  into wildly different equity paths (some robots roughly double, one nearly
  halves) — a path-dependency/butterfly effect, not a systematic edge from the
  fix. What *is* verified directly: holiday-Tuesday trade entries go from 0 in
  the old code to dozens per robot with the fix, confirming the mechanism now
  fires exactly when it should. This is promoted as a correctness fix, not a
  return-improving one.

## [0.11.2] — 2026-07-05

### Added
- **Liquidity floor on the SB robots (B-GATE-W3, D).** The SA/SV candidate gates
  enforce a $50M single-day dollar-volume floor (`LIQ_MIN_DV`); the SB gate had none,
  so B/D could enter $1-3M/day names. Added a **$30M floor on the 7-trading-day average**
  dollar volume (`LIQ_MIN_SB`, new `dollar_vol_avg7` panel column) — a fast-rotation
  strategy (~7-day median hold) should measure tradeable liquidity over the hold horizon,
  not a single spike-prone day. Validated in an isolated harness across 2022+/2023+/2024+
  windows: a clean win for both robots — B-GATE-W3 MaxDD −75%→−56% with return up; D return
  and Sharpe up with drawdown flat-to-better. SA/SV robots are untouched (a control). Live
  current holdings barely change (only thin names like NGNE drop out); the backtest curves
  improve. `robots.json` is regenerated by the pipeline.

### Changed
- **Robot liquidity/score knobs moved to `config.ini`.** `liq_min_dv`, `liq_min_sb`,
  `dv_avg_win`, and `score_floor` now live in a new `[robots]` section (typed via
  `RobotsConfig` in `config.py`), replacing hardcoded literals in `pipeline/robots.py`.
  No behavior change — the defaults equal the previous constants, so `robots.json` output
  is byte-identical. (The backtest-port `BT_STAGE` thresholds and the sector-tilt/quadrant-
  weight maps remain in code as a faithful port; they can move next if wanted.)

## [0.11.1] — 2026-07-05

### Fixed
- **Look-ahead bias in the robots' GICS sector-strength score.** `gics_panel`'s
  `strength_score` percentile-ranked each day's cumulative sector excess against the
  sector's *entire* history — future dates included — so a simulated pick's "sector
  strength" was computed with information that hadn't happened yet. Every robot whose
  score uses `gics_strength` (the SA multi-factor term, and multiplicatively the SB
  dashboard score) was affected, inflating the backtest. Switched to a point-in-time
  **expanding** percentile rank (`x.expanding(min_periods=1).rank(pct=True)`): today's
  value is ranked only among values up to today. The raw-sign `quadrant` (the
  STRONG/WARMING eligibility gate) and `delta_Y` were already point-in-time and are
  unchanged. **Live signals are unchanged** — on the last bar the expanding rank equals
  the full-history rank, so current holdings/candidates are identical; only the simulated
  history is corrected. Headline backtest stats drop accordingly (validated in an isolated
  harness that reproduces the prior numbers bit-for-bit before the fix): most notably
  B-GATE-W3's return roughly halves and its true MaxDD is ~−75% (vs ~−65% shown), while
  D improves and the SA family reshuffles ±8–20%; Robot C (score independent of sector
  strength) is unchanged, a clean control. `robots.json` is regenerated by the pipeline.
