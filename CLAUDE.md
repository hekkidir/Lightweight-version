...
- **Two serving paths, no build step.** (A) FastAPI ([server/app.py](server/app.py)) reads
  parquets → JSON live at `/api/*` (+ `/api/v1`). (B) **Static** — the dashboard JS
  ([frontend/data.js](frontend/data.js)) actually fetches `./data/*.json`, pre-generated by
  `pipeline/export_static.py` and committed under `frontend/data/`. GitHub Pages deploys
  `frontend/` directly (`.github/workflows/pages.yml`); the FastAPI catch-all also serves
  those same static files. So the `/api/*` routes are a parallel interface the current
...
| `pipeline/export_static.py` | Standalone (not wired into `run.py`): parquets → static `frontend/data/*.json` (incl. `stocks_detail.json`) for the GitHub Pages build. Run `python -m pipeline.export_static [data_dir] [out_dir]` |
